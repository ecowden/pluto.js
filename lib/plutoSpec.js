...
test('after bind(name).toInstance(instance), module.get(name) returns the instance', function* (t) {
  ...
  const actual = yield module.get('$injected')
...
test('bind.get(name) throws if the specified name is not mapped', function* (t) {
...
test.skip('Module.getAll([names]) accepts an array of names and returns a matching array of instances', function* (t) {
  // TODO deprecate getAll(...)?

  const bind = pluto()
  bind('a').toInstance('A')
  bind('b').toInstance('B')

  const actual = yield instance.getAll(['a', 'b'])
  t.deepEqual(actual, ['A', 'B'])
})

test.skip('Module.getAll([names]) throws if a name is unmapped', function* (t) {
  // TODO deprecate getAll(...)?
